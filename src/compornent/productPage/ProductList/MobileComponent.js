            ...
            <div className="tw-rounded-lg tw-bg-white tw-p-3 tw-mb-3 hover:tw-shadow-2xl tw-transition tw-ease-in-out">
                <div className="tw-flex-col ">
                    <div className="tw-flex">
                        <div className="tw-w-full tw-h-36 tw-border tw-border-gray-200">
                            ...
                            <div className="tw-flex tw-items-center">
                                ...
                                <div className="tw-h-[4.625rem] tw-ml-3 tw-flex tw-flex-col tw-justify-between">
                                    <div className="tw-flex tw-flex-col">
                                        <span className="tw-text-md tw-font-bold tw-truncate">{item.start_time}</span>
                                        <div className="tw-flex tw-items-center tw-ml-1">
                                            {/* <p className="tw-text-gray-500 tw-text-sm ">{item.startWard_name}</p> */}
                                        </div>
                                    </div>
                                    <div className="tw-flex tw-flex-col">
                                        <span className="tw-text-md tw-font-bold tw-truncate">{moment(item.end_time).utc(true).format("HH:mm")}</span>
                                        <div className="tw-flex tw-items-center tw-ml-1">
                    ...
                    <div className="tw-flex tw-flex-col tw-my-4">
                        ...
                        <div className="tw-flex tw-justify-between">
                            <div className='tw-w-full'>
                                ...
                                <p className="tw-py-1">{item.seat_empty > 0 ? `Số ghế trống ${item.seat_empty}` : <span>Hết ghế</span>}</p>
                            </div>
                            <div className='tw-w-full tw-flex tw-flex-col tw-justify-end'>
                                <p className="tw-text-xl tw-my-2 tw-text-center tw-font-black tw-text-green-700">{new Intl.NumberFormat('vi', { currency: 'VND', style: 'currency', }).format(item.price)}</p>
                                {item.seat_empty <= 0 ? null :
                                    <Link to={`/productdetail/${item.id}`}>
                                        <button className="tw-py-2 tw-bg-red-500 tw-w-full tw-text-white tw-rounded-lg">
                                            Chọn chuyến
                                        </button></Link>}
                            </div>
