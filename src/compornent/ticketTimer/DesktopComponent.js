...
const DesktopComponent = (props) => {
    const {timeTicket, pathname, localTicket, cancelTimeCountdown, checkLocal} = props
    return (
        <div>
             {timeTicket && checkLocal && pathname !== "/ticket" && pathname !== `/productdetail/${localTicket().id}` && pathname !== "/payment/success" ?
                <div className='tw-fixed tw-z-20 tw-mx-auto tw-bottom-[7rem] tw-right-0 tw-rounded-lg'>
                    ...
                    <p className='tw-rounded-lg tw-shadow-xl tw-text-xs tw-text-red-500 tw-my-2 tw-p-2 tw-bg-white'>
                        Gọi số 19001009 để giữ vé hoặc <Link to={`/productdetail/${localTicket().id}`}><span className='tw-text-green-500 tw-underline'>thanh toán lại</span></Link>  </p>
                </div> : null}
        </div>
    )
}
