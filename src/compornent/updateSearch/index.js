...
const UpdateSearch = () => {
    ...
    const provinceFilterStart = provinceFilter.filter(item => item.value == start).map(item => item.label)
    const provinceFilterEnd = provinceFilter.filter(item => item.value == end).map(item => item.label)
    const provinceFilterDate = provinceFilter.filter(item => item.value == date).map(item => item.label)
    return (
            ...
            <div className="formTrip tw-relative tw-z-20 tw-flex tw-items-center tw-w-3/4 tw-mx-auto">
                <div className="tw-flex tw-flex-grow tw tw-items-center tw-h-20 ">
                    <div className="tw-w-full tw-mr-2">
                        ...
                        <SelectForm placeholder={provinceFilterStart} className="tw-cursor-text" options={provinceFilter} /></div>
                    <div className="tw-w-full tw-mr-2">
                        <p className="tw-text-white">Điếm đến</p>
                        <SelectForm placeholder={provinceFilterEnd} className="tw-cursor-text" options={provinceFilter} />
                    </div>
                    <div>
                        <p className="tw-text-white ">Ngày đi</p>
                        <DatePicker className="tw-p-2 tw-rounded-lg tw-bg-green-700 tw-text-white" dateFormat="yyyy-MM-dd" /></div>
                </div>
